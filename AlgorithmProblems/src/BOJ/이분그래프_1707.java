        ...
        for (int k = 0; k < K; k++) {
            ...
            for (int i = 1; i <=V ; i++) {
                ...
                if (Colors[i]==null) {
                    ...
                    // q1이 빌 때까지 -> 사실상 q1, q2가 모두 빌 때까지 이므로 연결된 모든 vertex들을 탐색
                    while(!q1.isEmpty()) {
                        ...
                        for (int j : adj[v]) {
                            ...
                            // 색이 칠해지지 않았으면 color(v와 다른 색)로 칠하고 q2에 추가
                            if (Colors[j]==null) {
                                q2.add(j);
                                Colors[j]=color;
                            }
                        ...
                        if (q1.isEmpty()) {
                            color = change.get(color);	// q2의 정점은 color로 칠해져 있으므로 다음에는 change.get(color)로 칠해야함
                            q1=q2;
                            q2= new LinkedList<>();
                        }
                    }
